...
export const getAllData = async (offset) => {
  return instance.get(
    `/incendios-forestales/records?where=fecha_inicio%20>%20"2021-10-20"&order_by=fecha_inicio%20desc&limit=10&offset=${offset}`
  );
};
...
export const getFilteredData = async (query, offset) => {
  return instance.get(
    `/incendios-forestales/records?where=${query}&order_by=fecha_inicio%20desc&limit=10&offset=${offset}`
  );
};
